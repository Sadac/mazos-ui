...
setMazo({
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (mazo.nombre.trim() === "" || mazo.descripcion.trim() === "" || mazo.email.trim() === "") {
      setError(true);
      return;
    }
...
if (respuesta.message) {
  if(respuesta.message === 'El usuario no existe, intenta con otro email'){
    alert(`${mazo.email} no pertenece a ningun usuario`)
  }
}  
  setError(false);
    setModal(false);
    ...
    <Fragment>
      ...
      <Modal isOpen={modal}>
        ...
        <ModalBody>
          <form>
            {error ? (
              <p className="p-2 bg-danger rounded text-light">Todos los campos son obligatorios</p>
            ) : null}
            <input
              type="text"
